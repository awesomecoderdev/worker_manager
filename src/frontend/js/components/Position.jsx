...
const Position = ({searchPosition,setSearchPosition}) => {
  const [selected, setSelected] = useState(position[0])
  const [query, setQuery] = useState('')

  useEffect(() => {
    setSearchPosition(selected)
  }, [selected]);
  ...
  return (
    <div className="relative max-w-sm z-40 mx-2">
      <Combobox value={selected} onChange={setSelected}>
        <div className="relative mt-1">
          ...
          <Transition
            ...
            afterLeave={() => setQuery('')}
          >
            <Combobox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm scrollbar:!w-1.5 scrollbar:!h-1.5 scrollbar:bg-transparent scrollbar-track:!bg-slate-100 scrollbar-thumb:!rounded scrollbar-thumb:!bg-slate-300 scrollbar-track:!rounded supports-scrollbars:pr-2">
