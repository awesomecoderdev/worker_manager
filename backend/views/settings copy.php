...
<div class="w-full p-4" x-data="Settings()">
    ...
    <?php if (isset($_REQUEST["worker_settings"]) && wp_verify_nonce($_REQUEST['worker_settings'], 'worker_settings_action')) : ?>
        <?php if (isset($_REQUEST["user"]) && !empty($_REQUEST["user"])) : ?>
            ...
            $elemantorPage = update_option("worker_settings_elemantor_single_user", intval($_REQUEST["elemantorPage"]), true);

            ?>
            <?php if ($update_user || $update_account_page || $changeLogin || $changeRegister || $changeReset || $singleWorker  || $elemantorPage) : ?>
    ...
    <!-- template dropdown -->
    <p class="ml-1 my-1 italic text-slate-800 text-sm font-light"><?php _e("Select elemantor template.", "ac_worker_manager") ?></p>
    <div class="relative inline-block text-left">
        <div>
            <button @click="elemantorTemplate = !elemantorTemplate" type="button" class="flex justify-between w-full min-w-[16rem] max-w-[20rem] rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:border-primary-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50 " id="menu-button" aria-expanded="true" aria-haspopup="true">
                <span id="elemantorTemplateText" class="truncate max-w-[12rem]">
                    <?php echo $worker_settings_elemantor_single_user ? ucwords(get_the_title($worker_settings_elemantor_single_user)) : __("Select Elemantor Template", "ac_worker_manager"); ?>
                </span>
                <svg class="-mr-1 ml-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                    <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                </svg>
        ...
        <div x-show="elemantorTemplate" x-on:click.outside="elemantorTemplate = false" class="z-50 max-h-64 overflow-y-scroll overflow-x-hidden origin-top-right absolute left-0 mt-2 w-64 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
            <div class="py-1" role="none">
                <?php
                // elementor_library

                $template = new WP_Query([
                    'post_type'         => "elementor_library", //all pages
                    'posts_per_page'    => 9999,
                    'order'             => 'ASC',
                    'orderby'        => 'name',
                ]);
                ?>
                <?php if ($template->have_posts()) : ?>
                    <?php while ($template->have_posts()) : $template->the_post(); ?>
                        <?php if ($worker_settings_elemantor_single_user != get_the_ID()) : ?>
                            <span @click="changeElemantorPage($event)" class="truncate max-w-[15rem] cursor-pointer bg-white hover:bg-gray-100 text-slate-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" data-page="<?php the_ID(); ?>">
                                <?php echo ucwords(get_the_title()); ?>
                            </span>
                        <?php endif; ?>
                    <?php endwhile; ?>
                <?php else : ?>
                    <span @click="elemantorTemplate = !elemantorTemplate" class="cursor-pointer bg-white hover:bg-gray-100 text-slate-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1">
                        <?php _e("No template available.", "ac_worker_manager"); ?>
                    </span>
                <?php endif; ?>
            </div>
        </div>
    ...
    <form action="<?php echo admin_url("admin.php?page=ac_worker_manager_settings"); ?>" method="POST" class="relative space-y-1">
        ...
        <input type="hidden" name="elemantorPage" id="selectSlemantorPage" value="<?php echo get_option("worker_settings_elemantor_single_user"); ?>">
